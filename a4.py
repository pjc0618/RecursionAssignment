...
Phil Cipollina (pjc272) and Luke Marcinkiewicz(lam365)
October 24, 2017
"""
...
def is_valid_penmode(p):
    """
    Returns: True if p is a Pen with fill False; False otherwise.
...
def draw_spiral(w, side, ang, n, sp):
    ...
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert is_number(ang), report_error('ang is not a validnumber',ang)
    w.clear()
    t=cornell.Turtle(w)
    t.heading=0
...
def draw_spiral_helper(t, side, ang, n, sp):
    ...
    for x in range(0,n):
        side=side*(1+x)
        #print('side length of run'+ str(x) +'is' + str(side))
...
def multi_polygons_helper(t, side, k, n, sp):
    ...
    assert is_valid_length(side), report_error('side is not a valid length',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert type(k)==int and k>=1, report_error('k is not a valid number of polygons',k)
    assert (type(n)==int and n>=3), report_error('n is not a valid number of sides',n)
    color=t.color
    spd=t.speed
    direction=t.heading
...
def radiate_helper(t, side, n, sp):
    ...
    ang=360.0/n
    for _ in range(0,n):
        t.color=cornell.HSV(t.heading % 360.0, 1.0, 1.0)
        t.forward(side)
        t.move(0,0)
        t.left(ang)
    t.speed=spd
...
def triangle_helper(p, x, y, side, d):
    ...
    assert is_valid_length(side), report_error('side is not a valid number',
                                               side)
    assert is_valid_depth(d), report_error('d is not a valid depth',d)
    ...
    if d==0:
        ...
        triangle_helper(p,x,y,side/2.0,d-1)
        triangle_helper(p,x+side/2.0,y,side/2.0,d-1)
        triangle_helper(p,x+side/4.0,y+math.sqrt(3)*side/4.0,side/2.0,d-1)
...
def snowflake_helper(p, x, y, side, d):
    ...
    if d==0:
        ...
        dy=side/(3.0**.5)
        s=side/3.0
        snowflake_helper(p,x+dx,y,s,d-1)
        snowflake_helper(p,x-dx,y,s,d-1)
        snowflake_helper(p,x+side/3.0,y+dy,s,d-1)
        snowflake_helper(p,x+side/3.0,y-dy,s,d-1)
        snowflake_helper(p,x-side/3.0,y+dy,s,d-1)
        snowflake_helper(p,x-side/3.0,y-dy,s,d-1)
